...
(fn concat-strings-with-spaces [first second ...]
  (if (and (not first) (not second)) (values)
      ...
      (do ;; (print :concat-strings-with-spaces ((require :fennelview) [first second]))
        (concat-strings-with-spaces (.. first " " second ) ...))))

(var form->string nil)

(fn complex-form->string [form opener closer]
  ;; (print ((require :fennelview)) {: form : opener : closer})
  (.. opener
...
(fn create-form [form-type ...] (let [form [form-type ...]] form))
...
(fn token-stream->form-stream [token-stream]
  (let [boxed-token-stream (box-tokens token-stream)]
    (fn take-form [token-stream stack stack-length form-lengths whitespace-since-dispatch prefixed-with]
      ...
      (let [tts token-types
            ...
            begin-prefix
            (fn [bytes] (set prefixed-with (match bytes [35] :hashfn [44] :unquote [96] :quote)))]
        (match token-type

  ...
  (values
   (fn [parser-state]
     (if done nil
         ...
         (do (set chunk (chunk-stream parser-state))
...
;; (fn deref [self] (. self 1))
;; (fn list-to-string [] :TODO)

;; (local SYMBOL_MT {1 :SYMBOL :__tostring deref})
;; (local EXPR_MT {1 :EXPR :__tostring deref})
;; (local LIST_MT {1 :LIST :__tostring list-to-string})
;; (local SEQUENCE_MT [:SEQUENCE])

;; (local VARARG (setmetatable [:...] {1 :VARARG :__tostring deref}))
;; (fn varg [] VARARG)
;; (fn varg? [x] (and (= x VARARG) x))

;; (fn list [...] (setmetatable [...] LIST_MT))
;; (fn list? [x] (and (= :table (type x)) (= LIST_MT (getmetatable x)) x))


;; (fn sym [str]
;;   (let [s {1 str : scope}]
;;     (each [k v (pairs (or meta {}))]
;;       (when (= :string (type k))
;;         (tset s k v)))
;;     (setmetatable s SYMBOL_MT)))

;; (fn sym? [x] (and (= :table (type x)) (= SYMBOL_MT (getmetatable x)) x))
...
;; (fn sequence [...]
;;   (setmetatable [...] SEQUENCE_MT))
;; (fn sequence? [x]
;;   (and (= :table (type x))
;;        (= (getmetatable x) SEQUENCE_MT)
;;        x))

;; a "form" has the following shape:
;; [form-type ... values]
(local form-types (enum symbol string number sequence table list))
(fn escape-string-for-output [str] (str:gsub "[\1-\31]" #(.. "\\" ($:byte))))
...
(local form->string
  (do 
    ...
    (fn complex-form->string [form opener closer]
      (.. opener
          (or (concat-strings-with-spaces (map-values form->string (rest-values (unpack form)))) "")
          closer))
    (set form->string
         (fn form->string [form]
           (let [ret
                 (match (. form 1)
                   form-types.symbol (. form 2)
                   form-types.string (.. "\"" (escape-string-for-output (. form 2)) "\"")
                   form-types.number (tostring (. form 2))

                   form-types.list (complex-form->string form "(" ")")
                   form-types.table (complex-form->string form "{" "}")
                   form-types.sequence (complex-form->string form "[" "]"))]
             ret)))
    form->string))


(fn string-form [str] (create-form form-types.string (canonicalize str)))
(fn string-form-from-bytes [...] (string-form (string.char ...)))
(fn string-form-from-keyword-string-bytes [colon ...] (create-form form-types.string (string.char ...)))
(fn number-form-from-bytes [...] (create-form form-types.number (tonumber (string.char ...))))
...
(fn close-form-with-stack [stack bytes]
  (when (= (stack.length) 0)
    (error "unexpected closing delimiter " (string.char (unpack bytes))))
  (let [form (stack.pop)
        expected-closer
        (match form
          [fts.list] ")"
          [fts.sequence] "]"
          [fts.table] "}")]
    (let [closer (string.char (unpack bytes))]
      (when (not= expected-closer closer)
        (error (.. "unexpected closing delimiter " closer ", expected " expected-closer))))
    form))
      ...
      (let [(token-type bytes) (boxed-token-stream)
            ...
            (fn [form]
              (if (= (stack.length) 0)
                  ...
                  (let [parent-form (. stack (stack.length))]
                    (parent-form.push form))))]
        (match token-type
          tts.symbol (dispatch (symbol-form-from-bytes (unpack bytes)))
          tts.string (dispatch (string-form-from-bytes (unpack bytes)))
          tts.keyword-string (dispatch (string-form-from-keyword-string-bytes (unpack bytes)))
          tts.number (dispatch (number-form-from-bytes (unpack bytes)))

          tts.opener (open-form bytes)
          tts.prefix (open-prefix-form bytes)
          tts.closer (close-form bytes))
        (if should-return return-value (take-form))))
    (fn take-form [token-stream stack stack-length form-lengths whitespace-since-dispatch prefixed-with prefixed-at]
      (var should-return nil)
      (var ret nil)
      (local stack (or stack []))
      (local form-lengths (or form-lengths []))
      (var stack-length (or stack-length 0))
      (var whitespace-since-dispatch whitespace-since-dispatch)
      (var prefixed-with prefixed-with)
      (let [tts token-types
            (token-type bytes) (boxed-token-stream)
            dispatch-form
            (fn [form]
              (let [form
                    (if prefixed-with
                        (let [pw prefixed-with]
                          (set prefixed-with nil)
                          (list-form (symbol-form pw) form))
                        form)]
                (if (= stack-length 0) (do (set ret form) (set should-return true))
                    (let [current-form (. stack stack-length)
                          form-length (. form-lengths stack-length)
                          next-form-length (+ form-length 1)]
                      (tset current-form next-form-length form)
                      (tset form-lengths stack-length next-form-length)))))
            begin-complex-form
            (fn [bytes]
              (let [new-stack-length (+ stack-length 1)
                    empty-form (match bytes
                                 [40] (list-form)
                                 [91] (sequence-form)
                                 [123] (table-form))]
                (tset stack new-stack-length empty-form)
                (tset form-lengths new-stack-length 1)
                (set stack-length new-stack-length)))
            end-complex-form
            (fn [bytes]
              (when (= stack-length 0)
                (error (.. "unexpected closing delimiter " (string.char (unpack bytes)))))
              (let [(expected-form-type expected-closer)
                    (match bytes
                      [41] (values form-types.list ")")
                      [93] (values form-types.sequence "]")
                      [125] (values form-types.table "}"))
                    form (. stack stack-length)]
                (when (not= expected-form-type (. form 1))
                  (error (.. "unexpected closing delimiter " (string.char (unpack bytes)) ", expected " expected-closer)))
                (let [finished-complex-form (. stack stack-length)]
                  (tset stack stack-length nil)
                  (set stack-length (- stack-length 1))
                  (dispatch-form finished-complex-form))))
            begin-prefix
            (fn [bytes]
              (set prefixed-with (match bytes [35] :hashfn [44] :unquote [96] :quote)))
            ]
        (match token-type
          tts.symbol (dispatch-form (symbol-form-from-bytes (unpack bytes)))
          tts.string (dispatch-form (string-form-from-bytes (unpack bytes)))
          tts.keyword-string (dispatch-form (string-form-from-keyword-string-bytes (unpack bytes)))
          tts.number (dispatch-form (number-form-from-bytes (unpack bytes)))
          tts.whitespace (set whitespace-since-dispatch true)

          tts.opener (begin-complex-form bytes)
          tts.closer (end-complex-form bytes)
          tts.prefix (begin-prefix bytes)
          nil (set should-return true))
        (if should-return ret
            (take-form boxed-token-stream stack stack-length form-lengths whitespace-since-dispatch prefixed-with))))
    (fn [] (take-form boxed-token-stream))))
...
;;       token-stream (-> #(f:read chunk-size) chunk-stream->byte-stream byte-stream->token-stream)]
